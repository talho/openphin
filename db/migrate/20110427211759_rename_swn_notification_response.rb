class RenameSwnNotificationResponse < ActiveRecord::Migration
  def self.up
    ...
    execute "UPDATE message_notification_response SET message_id=view_han_alerts.distribution_id FROM view_han_alerts WHERE view_han_alerts.id=message_notification_response.alert_id" unless defined?(HanAlert).blank?
  ...
  def self.down
    ...
    execute "UPDATE message_notification_response SET alert_id=view_han_alerts.id FROM view_han_alerts WHERE view_han_alerts.distribution_id=message_notification_response.message_id" unless defined?(HanAlert).blank?
