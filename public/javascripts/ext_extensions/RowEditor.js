...
Ext.ux.grid.RowEditor = Ext.extend(Ext.Panel, {
    ...
    init: function(grid){
        ...
        this.on('canceledit', function(editor, saveChanges) {
          var s = grid.getSelectionModel().getSelected();
          if(typeof s != "undefined") {
            for(var i = 0; i < editor.items.items.length; i++) {
              if(!editor.items.items[0].allowBlank) {
                if(s.get(grid.store.fields.items[i].name) == "") {
                  grid.store.remove(s);
                  return;
                }
              }
            }
            for(i = 0; i < grid.store.fields.items.length; i++)
              editor.items.items[i].setValue(s.get(grid.store.fields.items[i].name));
          }
        });
    },
