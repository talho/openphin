(function() {
  var TabProxy = Ext.extend(Ext.dd.StatusProxy, {
    constructor : function(config) {
      ...
      this.el.addCls("dd-tabpanel-proxy");
    },
    reset : function(config) {
      TabProxy.superclass.reset.apply(this, arguments);
      this.el.addCls("dd-tabpanel-proxy");
      this.ghost.removeCls(["x-tab-strip-top", "x-tab-strip-bottom"]);
    }
  }), 
  TabDragZone = Ext.extend(Ext.dd.DragZone, {
    constructor : function(tp, config) {
      ...
      TabDragZone.superclass.constructor.call(this, tp.getTabBar().getEl(), config);
    },
    getDragData : function(e) {
      var tab = e.getTarget('.x-tab', 10),
          t = this.tabpanel.getTabBar().getChildByElement(tab);
      if(t && !t.close) {
        d = tab.cloneNode(true);
        Ext.fly(d).applyStyles({margin:'0',left:'0',top:'0'});
        d.id = Ext.id();
        return {
          ddel : d,
          tabpanel : this.tabpanel,
          repairXY : Ext.fly(tab).getXY(),
          item : t.card,
          index : this.tabpanel.items.indexOf(t.card)
        }
      }
      return false;
    },
    /*onInitDrag : function() {
      ...
      var strip = this.tabpanel.getTabBar().getEl(), ghostClass;
      ...
      this.proxy.getGhost().addCls(ghostClass);
      return true;
    },*/
  ...
  TabDropTarget = Ext.extend(Ext.dd.DropTarget, {
    constructor : function(tp, config) {
      this.tabpanel = tp;
      this.ddGroup = tp.ddGroup;
      TabDropTarget.superclass.constructor.call(this, tp.getTabBar().getEl(), config);
    },
    notifyDrop : function(dd, e, data) {
      var tp = this.tabpanel, t = tp.findTargets(e), i = tp.items.indexOf(t.item), activeTab = tp.getActiveTab();
      if(tp === data.tabpanel) {
        tp.suspendEvents();
      }
  ...
  Ext.define('Talho.ux.DragDropTabs', {
      alternateClassName: 'Ext.plugin.DragDropTabs',
      alias: ['plugin.dragdroptabs'],
      init : function(tp) {
        Ext.applyIf(tp, {
          ddGroup : "TabPanelDD",
        ...
        tp.on("render", onTabPanelRender, tp, {
          delay : 1
        });
