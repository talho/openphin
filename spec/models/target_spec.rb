...
describe Target do
  context "on create" do
    before do
      ...
      @audience = FactoryGirl.create(:audience, :users => [user])
    end
    
    it "should store a snapshot of the audience recipients" do
      FactoryGirl.create(:target, :audience => @audience).users.should include(@user)
    end
    
    it "should not include public users if item specifies not to" do
      item = mock_model(Document, :include_public_users? => false)
      target = FactoryGirl.create(:target, :audience => @audience, :item => item)
