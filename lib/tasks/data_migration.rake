...
begin
  ...
  task :dshs_data_migrate => :environment do
    ...
    FasterCSV.open(filename, :headers => true) do |records|
      records.each do |row|
        unless check_email(row['mail']) && !row['displayName'].blank?
          puts row
          next
        end
        rows << row
    ...
    finalrows.each do |row|
      ...
      last_name = (name == first_name ? "" : last_name(name))
      ...
      if user.valid? && !jurisdiction.nil?
        user.save!
        user.role_memberships << RoleMembership.create!(:user => user, :jurisdiction => jurisdiction, :role => Role.public)
      else
        puts row
      end
    end
