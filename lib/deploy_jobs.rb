namespace :sphinx do
  ...
  task :start_if_not, :roles => :jobs do
    unless rails_env == "test"
      ...
      cmd += "(bundle exec rake ts:index RAILS_ENV=#{rails_env} && "
      cmd += "bundle exec rake ts:start RAILS_ENV=#{rails_env})"
      run cmd
    end
  end
  ...
  task :rebuild, :roles => :jobs do
    unless rails_env == "test"
      begin
        run "cd #{previous_release}; bundle exec rake ts:stop RAILS_ENV=#{rails_env}"
