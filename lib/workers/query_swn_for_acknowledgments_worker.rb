class QuerySwnForAcknowledgmentsWorker < BackgrounDRb::MetaWorker
  ...
  def query(args = nil)
    Service::SWN::Alert::AlertNotificationResponse.active.acknowledge.map(&:alert).uniq.each do |alert|
      ...
      alert.alert_device_types.map(&:device).reject{|type| devices[type].blank?}.each do |type|
        ...
        if request.nil?
          SWN_LOGGER.info "Body does not contain a getNotificatio`nResultsResult element for SWN Notification Response with alert # #{alert.id}"
          return false
        end
        ...
        rcptStatus.each do |status|
          processAcknowledgmentStatus alert, status
        end
  ...
  def processAcknowledgmentStatus alert, rcptStatus = nil
