    ...
    def capture_to_db(report)
      ...
      if report.criteria.present?
        ...
        begin
          ...
          joins = "LEFT JOIN users ON invitees.email = users.email LEFT JOIN role_requests ON users.id = role_requests.user_id"
          index = 0
          result.invitees.joins(joins).order("role_requests.role_id ASC").each do |invitee|
            begin
              doc = id.clone
