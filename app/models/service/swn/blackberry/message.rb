    ...
    xml.swn(:sendNotification, xmlns => "http://www.sendwordnow.com/notification") do
      xml.swn(:pSendNotificationInfo) do
        xml.swn(:SendNotificationInfo) do
          xml.swn(:id, message.messageId + "-BLACKBERRY")
      ...
      message.Recipients.each do |recipient|
        xml.swn(:rcpt) do
          ...
          xml.swn(:contactPnts) do
            recipient.Devices.select{|d| d.device_type == "Blackberry PIN"}.each do |device|
              xml.swn(:contactPntInfo, :type => "Text") do
                xml.swn(:id, device.id)
                xml.swn(:label, "BlackBerryPINDevice")
                xml.swn(:address, "#{device.URN}@blackberry.sendwordnow.com")
              end
            end
          end
        end
