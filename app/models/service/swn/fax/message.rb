...
#    xml.instruct!
    ...
    xml.swn(:sendNotification, xmlns => "http://www.sendwordnow.com/notification") do
      xml.swn(:pSendNotificationInfo) do
        xml.swn(:SendNotificationInfo) do
          xml.swn(:id, message.messageId + "-Fax")
  ...
  def add_notification(xml)
    xml.swn(:notification) do
      provider = message.Behavior.Delivery.Providers.select{|c| c.name == "swn" && c.device == "Fax"}.first
      title = ""
      messagetext = ""
      ...
      message.Recipients.each do |recipient|
        xml.swn(:rcpt) do
          ...
          xml.swn(:contactPnts) do
            recipient.Devices.select{|d| d.device_type == "Fax"}.each do |device|
              xml.swn(:contactPntInfo, :type => "Text") do
                xml.swn(:id, device.id)
                xml.swn(:label, "Fax Device")
                xml.swn(:address,  "#{device.URN}@fax.sendwordnow.com")
              end
            end
          end
        end
