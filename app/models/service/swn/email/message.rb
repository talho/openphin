class Service::SWN::Email::Message < Service::SWN::Email::Base
  property :message
  property :username
  property :password
    ...
    xml.swn(:sendNotification, xmlns => "http://www.sendwordnow.com/notification") do
      xml.swn(:pSendNotificationInfo) do
        xml.swn(:SendNotificationInfo) do
          xml.swn(:id, message.messageId + "-Email")
      ...
      message.Recipients.each do |recipient|
        xml.swn(:rcpt) do
          ...
          xml.swn(:contactPnts) do
            recipient.Devices.select{|d| d.device_type == "E-mail"}.each do |device|
