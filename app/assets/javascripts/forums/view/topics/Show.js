...
Talho.Forums.view.Topics.Show = Ext.extend(Ext.Panel, {
  ...
  initComponent : function() {
    ...
    var store = new Ext.data.JsonStore({
        ...
        restful: true,
        idProperty: 'id',
        fields: ['id', 'content', 'formatted_content', 'poster_name', 'user_id', 'user_avatar', 'created_at', 'forum_id', 'locked_at',
    ...
    this.items.push({xtype: 'grid', itemId: 'grid', cls: 'topic-list-grid', header: false, border: true, 
      store: store,
      columns: [
        {xtype: 'templatecolumn', dataindex: 'poster_name', header: 'Poster', tpl: person_column_template, sortable: false},
        {xtype: 'templatecolumn', dataindex: 'formatted_content', header: 'Content', id: 'formatted_content',  tpl: content_column_template, sortable: false},
        {xtype: 'actioncolumn', align: 'center', width: 30, items: [{
        ...
        {xtype: 'actioncolumn', align: 'center', width: 30, sortable: false,
         ...
         getClass: function (v,meta,record) { 
           if (record.get('is_super_admin') || record.get('is_forum_admin') || record.get('is_moderator') || record.get('is_user_owned')) {
             if (!(record.get('is_super_admin') || record.get('is_forum_admin') || record.get('is_moderator')) && record.get('locked_at')) { Ext.getCmp('reply_button').addClass('x-hide-display');}            
            return 'x-action-col-cell'} 
          else { 
            if (record.get('locked_at')) {Ext.getCmp('reply_button').addClass('x-hide-display');}
            return 'x-hide-display';}}, scope: this
        },
        {xtype: 'actioncolumn', align: 'center', width: 30, sortable: false,
